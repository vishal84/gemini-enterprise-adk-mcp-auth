...
async def _patched_get_headers(
    ...
    """
    # Check for the new ge_auth_resource scheme first
    if (credential and 
        credential.http and 
        credential.http.scheme.lower() == "ge_auth_resource"):

        if hasattr(credential.http, 'ge_authentication_resource_auth_id'):
            auth_id = credential.http.ge_authentication_resource_auth_id
            access_token = tool_context.state.get(f"{auth_id}")
            ...
            if access_token:
                logger.debug(f"Using ge_auth_resource token for auth_id: {auth_id}")
                return {"Authorization": f"Bearer {access_token}"}
            else:
                logger.warning(f"No access token found for auth_id: {auth_id}")
                return None
        else:
            logger.error("ge_auth_resource scheme specified but no ge_authentication_resource found")
            return None
...
def apply_patch():
    ...
    logger.info("Applied ge_auth_resource patch to McpTool._get_headers")


def remove_patch():
    """Remove the patch and restore original functionality."""
    McpTool._get_headers = _original_get_headers
    logger.info("Removed ge_auth_resource patch from McpTool._get_headers")
